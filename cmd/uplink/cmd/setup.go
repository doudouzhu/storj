...
func cmdSetup(cmd *cobra.Command, args []string) (err error) {
	...
	if setupCfg.Enc.KeyFilepath == "" {
		kfpath, err := filepath.Abs(encryptionKeyFilepath)
		if err != nil {
			return err
		}
		override["enc.key-filepath"] = kfpath
	} else {
		override["enc.key-filepath"] = setupCfg.Enc.KeyFilepath
	}
...
Please enter numeric choice or enter satellite address manually [1]: `)
	...
	override["satellite-addr"], err = ApplyDefaultHostAndPortToAddr(
	...
	key, err := storj.NewKey(encKey)
	if err != nil {
		return err
	}

	err = SaveEncryptionKey(key, override["enc.key-filepath"].(string))
	if err != nil {
		return err
	}

...
type Config struct {
	...
	RepairInterval   time.Duration `help:"how frequently checker should audit segments" releaseDefault:"1h" devDefault:"0h0m5s"`
	IrrepairInterval time.Duration `help:"how frequently checker should audit segments" releaseDefault:"1h" devDefault:"0h0m1s"`
...
func (service *Service) irrepairProcess(ctx context.Context) error {
	...
	for {
		...
		if len(seg) == 0 {
			// irrepairable Q empty
			return nil
		}
		s := make([]int32, 1)
		service.Limiter.Go(ctx, func() {
			err := service.repairer.Repair(ctx, string(seg[0].GetPath()), s)
			if err != nil {
				zap.L().Error("repair failed", zap.Error(err))
			}
